# Energy Guarantee Claim Schema Candidate v2
...
Mainline `Guarantee` read:
- baseline vs Energy policy

Local-line read:
- baseline vs one explicitly named operator arm inside a narrow local line

Boundary:
- mainline guarantee work should default to `baseline vs Energy policy`
- local-line operator comparisons may still be used as bounded supporting lines, but they should not silently stand in for the mainline guarantee surface
...
- `comparison_scope`
  - mainline guarantee work:
    - baseline vs Energy policy under fixed conditions
  - local supporting line:
    - baseline vs one named operator arm under fixed conditions
- `supported_behavior`
