# Guarantee Downstream-Acceptance Boundary Surface v2

## Status
- Date: 2026-03-31
- Type: guarantee drafting memo
- Scope: restate the downstream-acceptance boundary surface under the restored
  manuscript-facing single-contract rule and boundary-first ontology
- Supersedes:
...
- Not a final manuscript freeze

## 1. Current bounded boundary read

The current downstream-acceptance boundary remains the relevant second surface
in the manuscript. Its truthful current manuscript-facing read is:
- mainline comparison:
...
- visible mainline regression:
  - on `accepted_direction_followthrough_present`, Gemini drops from
    `13 / 24` to `10 / 24` under the same mainline comparison surface that
    removes preacceptance rework burden
...
### 2.2 family_scope
- the carried `priority-resolution` family only

### 2.3 horizon_scope
- the first accepted-direction boundary inside that carried family only
...
- the downstream line still remains provider-sensitive beyond that shared gain,
  with Gemini accepted-direction followthrough regressing on the same mainline
  surface

### 2.7 explicit_failure_boundary
- this surface does not close as pair-wide downstream-quality support
- Gemini `accepted_direction_followthrough_present` drops from `13 / 24` to
  `10 / 24` on the mainline `baseline vs Energy policy` comparison
...
## 3. Manuscript-facing boundary sentence

Under the tested provider pair, within the carried `priority-resolution`
family, at the first accepted-direction boundary, and on the directly scored fields
`accepted_direction_followthrough_present`,
...
`preacceptance_rework_burden_absent`, but Gemini
`accepted_direction_followthrough_present` simultaneously drops from
`13 / 24` to `10 / 24` on the same mainline comparison surface. The imported
...
- an explicit downstream boundary surface
- not a filled supported claim unit
- not a replacement for the single supported claim unit carried elsewhere in
  the manuscript
- not a replacement for the separate side-topic supporting surface carried
