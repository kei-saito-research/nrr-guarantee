# Guarantee Phase 2 Downstream-Acceptance Closure Import Note v4
...
`v3` made the repaired downstream-acceptance surface easy to inspect inside the
repo, but it let that repaired auxiliary read replace the paper-level
comparison contract. `v4` restores the manuscript-facing second claim to the
mainline `baseline vs Energy policy` comparison required by the inherited
`Policy-Verification` protocol and keeps the repaired `NRR vs Ablation` read as
an auxiliary boundary check only.

## 2. Mainline carried claim

The current second claim is carried by the mainline downstream-boundary read
that stays on the matched `baseline vs Energy policy` comparison surface.
...
- manuscript use:
  - second claim closes on the mainline `baseline vs Energy policy` comparison
