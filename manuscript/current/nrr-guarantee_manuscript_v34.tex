...
acceptance boundary: frozen early ambiguity-turn resolution, downstream
acceptance under the inherited mainline comparison contract, and repaired
...
downstream-acceptance boundary that stays on the inherited mainline comparison
contract into one manuscript surface with explicit supported behavior, failure
boundary, and unsupported extension for each unit;
...
\item \texttt{guarantee\_phase2\_downstream\_acceptance\_closure\_import\_note\_v4\_2026-03-30.md}
\item \texttt{guarantee\_second\_claim\_unit\_fill\_v7\_2026-03-30.md}
...
\item \texttt{guarantee\_supporting\_layer\_evidence\_map\_v6\_2026-03-30.md}
\end{itemize}

Third, \texttt{results/reviewed\_phase2\_downstream\_acceptance\_readout\_v8/}
carries the imported reviewed auxiliary repaired surface for the second claim
unit:
\begin{itemize}
...
with the imported reviewed auxiliary repaired surface listed above.
...
\hline
Downstream acceptance boundary & \texttt{guarantee\_second\_claim\_unit\_fill\_v7\_2026-03-30.md} & \texttt{accepted\_direction\_followthrough\_present}, \texttt{rejected\_direction\_intrusion\_absent}, \texttt{preacceptance\_rework\_burden\_absent}, \texttt{appropriate} & Bounded support for preacceptance burden removal on matched \texttt{baseline vs Energy policy}; repaired \texttt{NRR vs Ablation} remains auxiliary & Keep Gemini residuals and the auxiliary Anthropic \texttt{Baseline} > \texttt{NRR} fact visible; do not collapse into pair-wide downstream-guarantee language \\
\hline
