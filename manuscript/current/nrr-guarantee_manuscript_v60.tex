...
\renewenvironment{quote}
  {\list{}{\leftmargin1em\rightmargin1em}\item\relax\small\sloppy}
  {\endlist}
\newcommand{\code}[1]{\path{#1}}

\title{\textbf{NRR-Guarantee: Bounded Commit/Defer Guarantee Surfaces and Explicit Downstream Boundaries}}
\author{
...
\begin{abstract}
This paper studies a bounded guarantee question for commit/defer behavior under
explicit provider, family, horizon, and observability conditions. It reports
one supported early ambiguity-turn result together with downstream-acceptance,
side-topic, and later-horizon limits. In the tested provider pair, the early
ambiguity-turn comparison moves from \texttt{0 / 24} to \texttt{24 / 24} on
both providers for \code{resolution_attempt_present} and \texttt{appropriate},
and from \texttt{8 / 24} to \texttt{24 / 24} for
\code{unilateral_commit_absent}. At the downstream-acceptance boundary, the
matched \texttt{baseline vs Energy policy} comparison supports bounded
\code{preacceptance_rework_burden_absent} improvement: both provider slices
move from \texttt{0 / 24} to \texttt{24 / 24}, while Gemini
\code{accepted_direction_followthrough_present} drops from \texttt{13 / 24} to
\texttt{10 / 24}. The auxiliary repaired \texttt{NRR vs Ablation} comparison
remains mixed (\texttt{Anthropic = inconclusive}, \texttt{Gemini = support};
Anthropic \texttt{Baseline} remains stronger than Anthropic \texttt{NRR} on
\texttt{appropriate}). The repaired first-step side-topic result reaches
\texttt{12 / 12} on both providers, but it is reported only as supporting
evidence because it depends on a local \texttt{baseline vs revised Energy
policy} comparison rather than on the paper's matched
\texttt{baseline vs Energy policy} comparison. Taken together, the evidence
supports one narrow guarantee claim with explicit downstream, side-topic, and
later-horizon limits rather than a provider-universal or long-horizon
guarantee.
\end{abstract}
...
~\cite{seshia2022verifiedai,desai2019soter}. This paper uses
...
claim whose provider scope, horizon scope, observability scope, and failure
boundaries are made explicit and auditable.

Within the NRR series, \texttt{Core} and \texttt{Phi} fixed the upstream
position-dependent identity and state-mapping results~\cite{saito2025nrr,saito2026phi}.
\texttt{IME}, \texttt{Transfer}, and \texttt{Coupled} then constrained the
interface, transfer, and dependency conditions~\cite{saito2026ime,saito2026transfer,saito2026coupled}.
Integrated \texttt{paper7} fixed the delayed-commitment comparison and
boundary-honesty basis, and \texttt{Energy} fixed the calibration/control basis
that the present manuscript inherits directly~\cite{saito2026principles,saito2026energy}.

This paper uses \texttt{Policy}, \texttt{Dynamics}, and
\texttt{Policy-Verification} as supporting inputs for a bounded
\texttt{Guarantee} claim. Its purpose is to state only those downstream
conclusions that remain supportable when the dependency chain is kept at
\texttt{paper7} $\rightarrow$ \texttt{Energy} $\rightarrow$ \texttt{Guarantee}
and when provider, family, horizon, observability, and failure-boundary limits
are made explicit.
...
does not replace the top-level goal. In this paper, the practical use of
...
The scope is intentionally narrow. The paper states only the bounded claim that
the current evidence supports, without promoting any provider-universal,
...
explicitly unresolved. In this paper, the evidence-source clause is restricted
to the supporting analyses and observability sources used for the present
\texttt{Guarantee} claim.

\paragraph{Series Extensibility Statement:}
NRR is a derivation-and-evaluation framework, not a closed recipe. The
evaluated policy/operator set is treated as an evaluated set under tested
conditions, not as an exhaustive set. The aim of this paper is not to claim
that these runs exhaust the guarantee question, but to provide one bounded
claim with explicit supporting evidence, explicit failure boundaries, and
comparison rules that later work can test under new providers, new families,
and longer horizons.
...
This paper sits at the end of the current downstream chain:

\begin{center}
\texttt{paper7} $\rightarrow$ \texttt{Energy} $\rightarrow$ \texttt{Guarantee}
\end{center}

\noindent The dependency split is fixed for interpretation. Integrated
\texttt{paper7} supplies the comparison-and-boundary-honesty basis.
\texttt{Energy} supplies the calibration/control basis and the imported
evidence used here.
...
condition validation evidence. \texttt{Guarantee} then states the bounded
supported, supporting, and boundary results that remain after those inputs are
combined under explicit conditions. Figure~\ref{fig:guarantee-dependency-structure}
summarizes that dependency structure.
...
supporting, and boundary results must be stated without inflating them into
unrestricted runtime branchwise comparison.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../figures/fig2_guarantee_centered_architecture_v1.png}
\caption{Dependency structure for the current \texttt{Guarantee} paper.
Integrated \texttt{paper7} and \texttt{Energy} remain the direct upstream
inputs, while \texttt{Policy}, \texttt{Dynamics}, and
\texttt{Policy-Verification} are supporting inputs rather than serial upstream
steps. \texttt{Guarantee} carries the bounded supported, supporting, and
boundary results stated here.}
\label{fig:guarantee-dependency-structure}
\end{figure}

\section{Supporting Inputs}

\subsection{Policy Input}

The current policy layer fixes the downstream state vocabulary
(\code{active_objective}, \code{suspended_objective},
\code{accepted_direction}, \code{unresolved_relation},
\texttt{quarantine}, \code{local_relation_block}) together with the minimal
...
cleanest current instance of \code{defer_with_resolution}: no content work
...
current instance of \code{commit_to_accepted_direction} in which the
honest carry-forward read is bounded support for
\code{preacceptance_rework_burden_absent} under the mainline comparison
...
first-step instance of \code{accepted_direction} becoming
\code{active_objective} while the prior line is held as
\code{suspended_objective} under \texttt{quarantine}, but in the current
...
boundary then marks the point where \code{unresolved_relation} handling
weakens and \code{local_relation_block} is no longer treated conservatively.
...
bounded removal of \code{preacceptance_rework_burden_absent}, but Gemini
\code{accepted_direction_followthrough_present} drops from
...
\section{Claim And Boundary Discipline}

Any supported claim, supporting result, or explicit boundary result in this
paper must disclose a bounded reporting specification with the following
fields:
...
contract. Supported claims, supporting results, and matched-condition boundary
results must fill all eight fields explicitly. The later-horizon boundary is
instead carried as a diagnostic one-arm later-checkpoint degradation result:
...
boundary-only rather than an additional supported claim. No scientific result
in this paper is valid if the fields required by its evidentiary contract are
omitted. This paper therefore inherits the current schema rule that the claim
must be chosen before outcome inspection and must remain narrow enough
to prevent provider-universal, family-universal, or long-horizon overreach.
Table~\ref{tab:guarantee-surface-schema} summarizes the supported,
supporting, and boundary results under that disclosure schema, and
Figure~\ref{fig:guarantee-claim-progression} shows how those results relate
across the current progression.
...
\hline
\shortstack[l]{Early ambiguity-\\turn} &
\shortstack[l]{Tested provider pair,\\frozen priority-resolution family,\\early decision boundary} &
\shortstack[l]{Non-unilateral priority\\resolution} &
\shortstack[l]{No downstream acceptance,\\side-topic/quarantine, or\\later-horizon extension} \\
\hline
\shortstack[l]{Downstream\\acceptance\\boundary} &
\shortstack[l]{Tested provider pair,\\priority-resolution family,\\first accepted-direction\\boundary} &
\shortstack[l]{Bounded support for preacceptance\\burden removal\\under matched baseline vs\\Energy-policy conditions} &
\shortstack[l]{Keep Gemini residuals and\\the auxiliary repaired read\\explicit; do not collapse\\into pair-wide or provider-\\universal downstream-\\guarantee language} \\
\hline
\shortstack[l]{Side-topic/quarantine\\supporting\\surface} &
\shortstack[l]{Tested provider pair,\\repaired side-topic family,\\first accepted side-topic\\workstart checkpoint} &
\shortstack[l]{Clean first-step side-topic\\workstart under explicit\\side-first acceptance\\on a local repaired\\comparison} &
\shortstack[l]{Keep it as supporting evidence\\only; do not upgrade\\the local repaired comparison\\into a second manuscript\\supported claim or later-\\horizon stability read} \\
\hline
\shortstack[l]{Later-horizon\\boundary} &
\shortstack[l]{Tested provider pair,\\priority-resolution horizon-\\stability family,\\later checkpoints\\\texttt{turn7}/\texttt{turn9}/\texttt{turn11},\\diagnostic one-arm later-\\checkpoint read} &
\shortstack[l]{Boundary-only read:\\relation preservation\\weakens before suspended-\\main quarantine breaks} &
\shortstack[l]{Keep later-horizon as\\diagnostic boundary reporting\\only; no matched-\\comparison fill,\\no fourth supported claim,\\and no provider-universal\\or long-horizon extension} \\
\hline
\end{tabular}
}
\caption{Current supported, supporting, and boundary surfaces in
\texttt{Guarantee}.}
\label{tab:guarantee-surface-schema}
\normalsize
\end{table}

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../figures/fig1_supported_claim_progression_v1.png}
...
with the downstream-acceptance surface marking the first accepted-direction
boundary inside that family, while the side-topic/quarantine read belongs to a
separate repaired family and is carried only as supporting evidence. The later-horizon region is a separate
...
\label{fig:guarantee-claim-progression}
\end{figure}

\subsection{First Supported Claim Unit}

The first explicit supported claim unit is:
...
The downstream-acceptance boundary surface is:
...
\item provider scope:
  ...
  \texttt{gemini-2.0-flash})
\item family scope:
  the carried \texttt{priority-resolution} family only
\item horizon scope:
...
\begin{quote}
Under the tested provider pair, within the carried \texttt{priority-resolution}
...
The repaired first-step side-topic / quarantine supporting surface is:
...
The scientific argument in this paper draws on three evidence layers.
Supporting texts provide the inherited program design, policy, dynamics, and
protocol vocabulary used here. Analysis notes document the readings used for
the early ambiguity-turn claim, the downstream-acceptance boundary, the
repaired first-step side-topic / quarantine result, the later-horizon
boundary, and the policy/dynamics interpretation layer. An imported reviewed
readout preserves the repaired \texttt{NRR vs Ablation} downstream comparison
as auxiliary boundary evidence only.

Detailed filenames are listed in \texttt{reproducibility.md} and in the
supporting evidence map rather than reproduced in the main text. That
separation keeps the paper focused on the scientific interpretation while still
making the evidence traceable.

The supporting bundle records provenance and inherited dependencies, but it is
not the sole basis for every result reported here. In particular, the older
downstream-acceptance memo preserved in
\texttt{supporting\_layer\_sources/current/} remains a pre-repair historical
read. The downstream-acceptance section instead relies on the closure import
note, the boundary note, the baseline-vs-Energy comparison memo with its four
run-annotation CSVs, and the imported reviewed auxiliary comparison listed
above. The same evidence bundle also includes the repaired side-topic
comparison memo with its four run-annotation CSVs, together with the
later-horizon two-provider memo and scored read notes needed to inspect that
boundary in detail.
...
The third \texttt{Guarantee} entry point is the repaired first-step side-topic
/ quarantine supporting result. Under matched baseline
...
Relation Collapse} appears. Table~\ref{tab:guarantee-provider-residuals}
summarizes the provider-specific residual structure that keeps the current
\texttt{Guarantee} claims bounded.

\begin{table}[ht]
\centering
\small
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.16\textwidth}|>{\raggedright\arraybackslash}p{0.22\textwidth}|>{\raggedright\arraybackslash}p{0.22\textwidth}|>{\raggedright\arraybackslash}p{0.22\textwidth}|}
\hline
...
\hline
Downstream acceptance boundary & Mainline burden-removal support is clean; auxiliary repaired read stays \texttt{inconclusive} and keeps \texttt{Baseline} stronger than \texttt{NRR} on \texttt{appropriate} & Mainline burden-removal support is present, but \code{accepted_direction_followthrough_present} drops from \texttt{13 / 24} to \texttt{10 / 24}; auxiliary repaired read reaches \texttt{support} & Keep the claim at bounded burden-removal support under the mainline contract; use the repaired read only as boundary evidence, not as a replacement comparison rule \\
\hline
First-step side-topic / quarantine & Lift is visible, with strongest incremental gain on \code{C_incident_response} & Lift is also visible, again with the main gain concentrated on \code{C_incident_response} & Treat the result as template-bounded improvement, not uniform side-topic stability \\
\hline
...
\texttt{Guarantee} claims bounded.}
\label{tab:guarantee-provider-residuals}
\normalsize
\end{table}

\section{Later-Horizon Failure Boundary}

The later-horizon boundary surface is:
...
\item observability scope:
  \code{chosen_side_direction_progress_present},
  \code{user_detail_use_present},
  \code{unresolved_relation_preserved_or_narrowly_checked},
  \code{suspended_main_reference_absent},
  \code{starter_question_repeat_absent}, and \texttt{appropriate}
\item comparison scope:
...
\item explicit failure boundary:
  ...
  the clearest shared failure surface is \code{C_incident_response} at
...
later-horizon observability fields listed above, the
...
is the \code{C_incident_response} template at \texttt{turn9}, where both providers
...
The materials retained for this paper are intentionally narrow:
\begin{itemize}
...
\item bounded claim, supporting, and boundary drafting memos
\item an imported reviewed downstream-acceptance readout for the
...
\item bundled supporting-layer source files
\item stable build and verification wrappers
\end{itemize}

\begin{sloppypar}
These materials expose both the paper and the specific sources used to justify
its bounded conclusions. Checksum verification runs on the tracked artifact
set, and the documented build wrapper can produce a PDF in this environment
through a source-rendered fallback when cached TeX resources are incomplete.
Full typeset reproduction therefore remains environment-dependent here, but
the tracked materials are organized so that the bounded claim, the reviewed
downstream-boundary readout, and the supporting sources can be inspected
without relying on workspace-only links.
\end{sloppypar}
...
This paper also remains deliberately narrow. It is based on
...
discipline that keeps the paper from overstating what the current evidence can
support.

This staging matters for how the results should be read. The paper is
...
shows \code{accepted_direction_followthrough_present} dropping from
\texttt{13 / 24} to \texttt{10 / 24} and the auxiliary repaired
...
These results should also be read as a reusable bounded test rather than as the
last word on guarantee behavior. What is fixed here is not an exhaustive
...
The paper is limited to a narrow tested-provider surface: two
provider slices, one supported early-control claim, one explicit downstream
boundary, one side-topic supporting surface, and one later-horizon boundary
read rather than a broad cross-provider or long-horizon guarantee.

The downstream evidence also remains non-uniform. The repaired downstream-
acceptance boundary supports bounded preacceptance-burden removal under matched
\texttt{baseline vs Energy policy} conditions, but Gemini still shows reduced
\texttt{accepted\_direction\_followthrough\_present}, and the auxiliary repaired
\texttt{NRR vs Ablation} surface remains mixed, with Anthropic still
\texttt{inconclusive}. The behavior-first rerun likewise remains secondary
evidence only and does not establish pair-level continuation-quality
superiority. These results should therefore be read as a bounded test rather
than as an exhaustive guarantee theorem or final cross-provider closure.

\section{Conclusion}

This paper establishes a bounded \texttt{Guarantee} claim. It reports one
supported early ambiguity-turn result, carries the downstream-acceptance
comparison as an explicit boundary with bounded support for
\texttt{preacceptance\_rework\_burden\_absent} under matched
\texttt{baseline vs Energy policy} conditions, keeps the repaired
\texttt{NRR vs Ablation} comparison as an auxiliary boundary comparator,
reports the repaired first-step side-topic/quarantine result only as
supporting evidence under matched \texttt{baseline vs revised Energy policy}
conditions, and keeps the later-horizon limit explicit as boundary reporting
rather than as an additional supported claim. The result is closer to a bounded
verification or assurance statement over explicit observables than to a
universal correctness claim~\cite{seshia2022verifiedai,desai2019soter}. Early
control support does not automatically extend into pair-wide downstream
acceptance, the repaired downstream boundary remains limited to burden removal
rather than pair-wide downstream quality, and the Anthropic-side auxiliary
comparison keeps \texttt{Baseline} stronger than \texttt{NRR} on
\texttt{appropriate}. Broader provider coverage, broader downstream family
coverage, and later-horizon closure remain outside the present claim.
...
\newblock Unpublished manuscript, 2026.
\url{https://github.com/kei-saito-research/nrr-energy}.
