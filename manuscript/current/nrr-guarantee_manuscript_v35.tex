...
manuscript integrates two supported claim surfaces and two explicit boundary
surfaces: frozen early ambiguity-turn resolution, downstream acceptance under
the inherited mainline comparison contract, repaired first-step side-topic/
quarantine workstart, and later-horizon relation-preservation failure
reporting. On the current tested-provider
...
downstream-acceptance boundary supports bounded
...
\texttt{0 / 24} to \texttt{24 / 24}, while Gemini still retains visible
first-followthrough residuals), the auxiliary repaired
...
providers. The result is a narrow downstream paper surface: two supported
guarantee surfaces plus explicit downstream and later-horizon boundaries.
Across those surfaces, the honest read is therefore non-uniform: the
...
reusable test surface: bounded supported claims, explicit boundary surfaces,
explicit comparison rules, provider-sliced reporting, and visible failure
boundaries that later readers can rerun, challenge, and extend under new
providers, new families, and longer horizons.
...
downstream-acceptance boundary and one later-horizon failure boundary into one
manuscript surface with explicit supported behavior, failure boundary, and
unsupported extension for each surface;
\item it imports policy and dynamics vocabulary only at the level needed to make
those supported claims and boundary surfaces interpretable without widening them
into theorem-level claims;
...
still retains first-followthrough residuals, and the auxiliary repaired
...
Hold}. The later-horizon boundary is best read as weakening
...
Any later supported claim or explicit boundary surface in this paper must fill a
single bounded reporting surface with the following fields:
...
\hline
\end{tabular}
\caption{Current supported and boundary surfaces in
\texttt{Guarantee}.}
\end{table}
...
\caption{Supported claim progression and explicit boundary surfaces. Claim 1 and
the downstream-acceptance boundary occupy different horizon points within the
same \texttt{priority-resolution} family, while Claim 2 belongs to the
separate first-step side-topic/quarantine family. The later-horizon region is
shown as a boundary-only read rather than as an additional supported claim.}
\end{figure}
...
\item explicit failure boundary:
  ...
  continuation stability; Gemini still retains first-followthrough residuals,
  and the auxiliary repaired read remains mixed
\item unsupported extension:
...
\texttt{preacceptance\_rework\_burden\_absent}, but Gemini still retains
partial first-followthrough residuals on part of the tested surface. The
...
\subsection{Second Supported Claim Unit}

The second explicit supported claim unit in the current manuscript is:
...
\item \texttt{guarantee\_first\_claim\_unit\_fill\_v3\_2026-03-30.md}
\item \texttt{guarantee\_phase2\_downstream\_acceptance\_closure\_import\_note\_v5\_2026-03-30.md}
\item \texttt{guarantee\_downstream\_acceptance\_boundary\_surface\_v1\_2026-03-30.md}
...
\item \texttt{guarantee\_behavior\_first\_manipulation\_check\_import\_note\_v2\_2026-03-30.md}
\item \texttt{guarantee\_supporting\_layer\_evidence\_map\_v7\_2026-03-30.md}
\end{itemize}

Third, \texttt{results/reviewed\_phase2\_downstream\_acceptance\_readout\_v8/}
carries the imported reviewed auxiliary repaired surface for the downstream-
acceptance boundary:
\begin{itemize}
...
auxiliary repaired surface listed above.
...
\hline
Behavior-first manipulation check & Tested provider pair (\texttt{claude-sonnet-4-6}, \texttt{gemini-2.0-flash}) & First next-move framing only & opening-comparison framing separation, direct relation work retention, and the visible Anthropic \texttt{D2} collapse row \\
\hline
...
\hline
\textbf{Surface} & \textbf{Primary source artifact} & \textbf{Direct observability fields} & \textbf{Current read in this paper} & \textbf{Boundary kept explicit} \\
\hline
...
\hline
Downstream acceptance boundary & \texttt{guarantee\_phase2\_downstream\_acceptance\_closure\_import\_note\_v5\_2026-03-30.md}; decisive basis: \texttt{energy\_policy\_verification\_downstream\_boundary\_baseline\_vs\_energy\_policy\_comparison\_memo\_v1\_2026-03-17.md} (+ 4 run-annotation CSVs) & \texttt{accepted\_direction\_followthrough\_present}, \texttt{rejected\_direction\_intrusion\_absent}, \texttt{preacceptance\_rework\_burden\_absent}, \texttt{appropriate} & Bounded burden-removal support on matched \texttt{baseline vs Energy policy}; repaired \texttt{NRR vs Ablation} remains auxiliary boundary evidence & Keep Gemini residuals and the auxiliary Anthropic \texttt{Baseline} > \texttt{NRR} fact visible; do not collapse into pair-wide downstream-guarantee language \\
\hline
...
\hline
Behavior-first manipulation check & \texttt{guarantee\_behavior\_first\_manipulation\_check\_import\_note\_v2\_2026-03-30.md} & opening-comparison framing separation, direct relation work retention, visible Anthropic \texttt{D2 = 1 / 10} collapse row & Secondary bounded behavioral-separation note only & Not a filled claim unit or pair-level continuation-quality superiority claim \\
\hline
...
\hline
\end{tabular}
\caption{Source-to-surface trace for the bounded surfaces carried by the current
\texttt{Guarantee} manuscript.}
\end{table}

\subsection{Policy/Dynamics Interpretation Layer}

Taken together, these supported and boundary surfaces define a bounded
progression rather than a universal recipe. At the earliest boundary, the
...
Continuation Hold}. At the repaired first-step
...
\item bounded claim and boundary drafting memos
...
retains partial first-followthrough residuals and the auxiliary repaired
...
pretend the paper-level mainline comparison has already propagated forward into
pair-wide downstream quality. It is the current bounded result.
...
explicit supported claims, explicit boundary surfaces, explicit comparison
surfaces, provider-sliced reporting, and visible failure boundaries. That
...
quarantine surface, carries the downstream-acceptance boundary forward as
bounded support for \texttt{preacceptance\_rework\_burden\_absent} on the
mainline \texttt{baseline vs Energy policy} comparison while keeping the
repaired \texttt{NRR vs Ablation} read as an auxiliary comparator, and keeps the
