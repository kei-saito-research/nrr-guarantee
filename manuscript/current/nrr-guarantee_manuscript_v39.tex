...
its provider, family, horizon, observability, explicit failure boundary, and
unsupported extension remain explicit, but it is not reported as a matched
comparison fill or as an additional supported claim. No carried scientific
...
\hline
Later-horizon boundary & Tested provider pair, same-family later checkpoints only, diagnostic one-arm later-checkpoint read & No additional supported guarantee claim & Failure appears as weakening relation preservation and premature relation collapse, especially on \texttt{C\_incident\_response} at \texttt{turn9} \\
\hline
\end{tabular}
...
is carried only as supporting evidence. The later-horizon region is shown as a
...
claim or as a matched-condition comparison surface. The current measured horizon
family is reported only as a diagnostic one-arm boundary read that marks where
the carried first-step side-topic supporting surface stops being safe to
extend.
The limited fact that the first side-topic switch can survive into the next
checkpoint, and that suspended-main quarantine can still be visible there, is
included only to locate the onset of failure. Later degradation remains real,
template-dependent, and not uniformly distributed across providers.
...
\newblock GitHub repository: \texttt{https://github.com/kei-saito-research/nrr-principles}; bundled authority copy in the current review surface \texttt{supporting\_layer\_sources/current/paper7\_integrated\_manuscript\_v0\_3\_2026-03-29.tex}, 2026.

\bibitem{saito2026energy}
K.~Saito,
\newblock ``NRR-Energy: Calibration of Commit/Defer Utility Under Existing Paired Inputs,''
\newblock GitHub repository: \texttt{https://github.com/kei-saito-research/nrr-energy}; bundled authority copy in the current review surface \texttt{supporting\_layer\_sources/current/nrr-energy\_manuscript\_v11.tex}, 2026.
