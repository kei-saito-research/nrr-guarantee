...
\caption{Current downstream ownership split. \texttt{Policy},
\texttt{Dynamics}, and \texttt{Policy-Verification} remain supporting layers,
while \texttt{Guarantee} is the current claim-owning paper surface. The
manuscript-facing upstream spine is fixed at \texttt{paper7 -> Energy ->
Guarantee}; this figure only fixes the downstream ownership split inside that
spine.}
\end{figure}
...
The manuscript's evidence surface is split across two repo-local locations.
...
notes that fix the current bounded reads:
\begin{itemize}
\item \texttt{guarantee\_first\_claim\_unit\_fill\_v2\_2026-03-20.md}
\item \texttt{guarantee\_phase2\_downstream\_acceptance\_closure\_import\_note\_v1\_2026-03-30.md}
\item \texttt{guarantee\_second\_claim\_unit\_fill\_v4\_2026-03-30.md}
...
\item \texttt{guarantee\_supporting\_layer\_evidence\_map\_v3\_2026-03-30.md}
\end{itemize}

This split is deliberate. The support bundle carries fixed upstream dependency
texts; it is not by itself the manuscript-facing proof surface for every later
claim unit. In particular, the older downstream-acceptance memo preserved in
\texttt{supporting\_layer\_sources/current/} remains a pre-repair historical
read. The current second claim unit is instead anchored by the repo-local
closure import note and claim-unit fill listed above.
...
\texttt{Guarantee} entry point. The repo-local closure import note records the
review-closed repaired readout on the packaged downstream-acceptance artifact:
Anthropic remains inconclusive while Gemini reaches support. The truthful
carry-forward read is therefore a two-provider mixed result rather than a
pair-wide uniform outcome. This surface remains manuscript-useful only as
...
without workspace-only links. Stripped comparison bundles are not the same
thing as this tracked repository surface.
