...
dependency spine is kept at \texttt{paper7} $\rightarrow$ \texttt{Energy}
$\rightarrow$ \texttt{Guarantee} and when
...
imported under explicit conditions. Figure~\ref{fig:guarantee-dependency-structure}
summarizes that manuscript-facing dependency structure.
...
supporting, and boundary surfaces under that disclosure schema, and
Figure~\ref{fig:guarantee-claim-progression} shows how those surfaces relate
across the current manuscript-facing progression.
...
\paragraph{Prospective-Killer Bridge Note}

A separate prospective-killer pilot on the repaired English bridge surface does
not enter this paper as a supported \texttt{Guarantee} claim unit or as a
boundary surface. Its narrower manuscript role is to show that prompt-level
\texttt{NRR-style} control can materially improve multi-turn behavior under the
tested English scenario protocol while still leaving an explicit later-
followthrough residual, which makes the pilot useful here only as a bounded
bridge note rather than as a new guarantee result. In the repaired full run,
overall \texttt{final\_success\_rate} moves from \texttt{0.802} to
\texttt{0.904} and \texttt{NRR} \texttt{final\_success\_rate} moves from
\texttt{0.472} to \texttt{0.769}; the main gain is in
\texttt{downstream\_acceptance}, where \texttt{DA / nrr} moves from
\texttt{0.083} to \texttt{0.861}, while \texttt{EA / nrr} remains strong
(\texttt{0.861 -> 0.889}) and \texttt{LF / nrr} improves only partially
(\texttt{0.472 -> 0.556}). The explicit carried-over residual
\texttt{LF\_002 / gemini / nrr} remains inside the scored set. The associated
\texttt{Gemini-only logprobs} read remains mechanistic sidecar evidence only.
Its role in this manuscript is therefore a secondary bounded bridge note inside
the evidence-import discussion rather than a main claim unit, a formal carried
surface, or proof that a non-softmax \texttt{NRR-native} architecture is
already established.
...
The prospective-killer pilot remains outside this formal surface list and is
carried only as a secondary bounded bridge note.
...
\item a secondary prospective-killer bridge note
...
The repaired prospective-killer pilot sharpens that boundedness rather than
relaxing it. Its correct manuscript role is a secondary bridge note inside the
evidence-import discussion: it shows that prompt-level \texttt{NRR-style}
control can materially improve multi-turn behavior on the tested English bridge
surface while still leaving later followthrough as the main residual family.
But that bridge read should not be overread as a broader downstream-quality or
mechanistic-closure result. The current evidence does not prove that
\texttt{softmax} alone explains every remaining failure, the
\texttt{Gemini-only logprobs} slice remains mechanistic sidecar evidence only,
and the current package still falls short of establishing a non-softmax
\texttt{NRR-native} architecture. Keeping those limitations adjacent to the
reported pilot result is part of the same bounded-claim discipline rather than
an afterthought.
...
\texttt{inconclusive}. The prospective-killer pilot likewise remains secondary
evidence only: it supports a bounded bridge read from prompt-level behavioral
improvement toward future architectural motivation, but it does not establish
pair-level continuation-quality superiority, provider-balanced mechanistic
proof, or a non-softmax architecture claim. The current package should
therefore be read as a reusable bounded test surface rather than as an
exhaustive guarantee theorem or final cross-provider closure.
...
\newblock Unpublished manuscript. Repository cited for current public surface,
2026.
...
\newblock Unpublished manuscript. GitHub repository and bundled authority copy
in the active review surface, 2026.
